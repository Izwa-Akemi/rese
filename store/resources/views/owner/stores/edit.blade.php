        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="post" action="{{ route('owner.stores.update',['store' => $store->id])}}" enctype="multipart/form-data">
                    @method('PUT')      
                    @csrf
                        <div class="-m-2">
                            <div class="p-2 sm:w-1/2 mx-auto">
                                <div class="relative">
                                    ...
                                    <input type="text" id="name" name="name" value="{{$store->name}}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                </div>
                            </div>
                            <div class="p-2 sm:w-1/2 mx-auto">
                                <div class="relative">
                                    <label for="image" class="leading-7 text-sm text-gray-600">店舗画像</label>
                                    <img src="/storage/shops/{{$store->filename}}" alt="">
                                    <input type="file" accept="image/png,image/jpeg,image/jpg" id="image" name="image" value="{{ $store->filename }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                </div>
                            </div>
                            <div class="p-2 sm:w-1/2 mx-auto">
                                <div class="relative">
                            ...
                            <div class="p-2 sm:w-1/2 mx-auto">
                                <div class="relative">
                                    ...
                                    <select name="genre_id" id="genre_id" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        ...
                                        <option value="{{ $genre->id}}" @if( $genre->id == $store->genre_id )selected @endif>{{ $genre->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="p-2 sm:w-1/2 mx-auto">
                                <div class="relative">
                                    ...
                                    <textarea id="detail" name="detail" rows="10" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $store->detail }}</textarea>
                                </div>
                            </div>
                    ...
                    <form id="delete_{{ $store->id }}" method="post" action="{{ route('owner.stores.destroy',['store' => $store ->id])}}">
                        @csrf
                        @method('delete')
                        <div class="p-2 w-full flex justify-around mt-32">
                            <a href="#" data-id="{{$store->id}}" onclick="deletePost(this)" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">削除</a>
                        </div>
                    </form>
                </div>
            </div>
